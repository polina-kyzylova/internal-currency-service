...
export default function CFOownersAutoList({ errors, register, title, setValue, getValues }) {
    ...
    useEffect(() => {
        ...
        (async () => {
            let x = await getUsers(`/users/fsc-owners?username=${getValues('cfo_owner')}&page=1&size=10`);
            if (active) {
                let result = x.data.users.filter((item) => item.username !== username);
                setOptions([...result])
                console.log('данннные ', result)
            }
        })();
    ...
    return (
        <div className='transaction-form-input-box'>
            ...
            <Autocomplete
                ...
                options={options}
                autoHighlight
                getOptionLabel={(option) => `${option.username} ${option.surname} ${option.name} ${option.lastname}`}
                    ...
                    return (
                        <Box key={key} component="li" {...optionProps}>
                            ...
                            <div className='transaction-target-user-container'>
                                <p className='transaction-target-username'>
                                    {option.username}
                                </p>
                ...
                renderInput={(params) => (
                    <TextField
                        ...
                        variant="standard"
                        error={errors.cfo_owner ? true : false}
                        {...register("cfo_owner", { required: true, })}
